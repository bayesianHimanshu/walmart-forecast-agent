...
stores = pd.read_csv("stores.csv", lineterminator="\r")
...
df = (
    train.merge(pairs, on=["Store", "Dept"])
...
def wmae(y, f, hol):
    w = np.where(hol, 5, 1)
    return float(np.sum(w * np.abs(y - f)) / np.sum(w))


def mase(y, f, ytr, m=52):
    d = (
        np.mean(np.abs(ytr[m:] - ytr[:-m]))
        if len(ytr) > m
        else np.mean(np.abs(np.diff(ytr)))
    )
...
def m_naive(h, exf, H):  # seasonal-naive (52w); fallback last value
    ...
    return (
        np.array([y[-52] if len(y) >= 52 else y[-1] for _ in range(H)])
        if len(y) >= 52
...
def m_prophet(h, exf, H):
    try:
        ...
        mp.add_regressor("hol")
        mp.fit(d)
        fut = pd.DataFrame({"ds": exf.Date.values})
        fut["hol"] = fut.ds.isin(pd.to_datetime(HOL)).astype(int)
...
def m_gbm(h, exf, H):  # lightgbm w/ lags + calendar + exog (proxy for ML.FORECAST)
    ...
    for i in range(H):
        row = exf.iloc[[i]].copy()
        tmp = pd.concat([cur, row], ignore_index=True)
        tmp = fe(tmp)
...
MODELS = {
    "SEASONAL_NAIVE": m_naive,
    "SARIMA": m_sarima,
    "PROPHET": m_prophet,
    "SNOWFLAKE_GBM": m_gbm,
}
H = 8
ORIGINS = [24, 16, 8]  # weeks-from-end; each forecasts next 8w (expanding train)
...
series_list = [
    ...
    "37_5",
    "20_38",
]  # holiday-driven, flat, erratic, high-vol, small-store, smooth
for s in series_list:
    g = df[df.series == s].reset_index(drop=True)
    n = len(g)
    for off in ORIGINS:
        cut = n - off
        if cut < 60:
            continue
        hist = g.iloc[:cut]
        win = g.iloc[cut : cut + H]
        if len(win) < H:
            continue
        exf = win[["Date"] + EXO].reset_index(drop=True)
        yv = win.Weekly_Sales.values
        holv = win.Date.isin(pd.to_datetime(HOL)).values
...
agg = (
    res.groupby(["series", "model"])
    .agg(wmae=("wmae", "mean"), mase=("mase", "mean"), smape=("smape", "mean"))
    .reset_index()
)
...
for s, gg in agg.groupby("series"):
    ...
    naive = gg.loc["SEASONAL_NAIVE", "wmae"]
    best = gg.wmae.idxmin()
    bestw = gg.wmae.min()
    ...
    champ.append(
        dict(
            series=s,
            champion=chosen,
            champ_wmae=gg.loc[chosen, "wmae"],
            naive_wmae=naive,
            impr_vs_naive=round(impr, 1),
        )
    )
...
print("\nsample champions:")
print(
    champ.sort_values("impr_vs_naive", ascending=False).head(10).to_string(index=False)
)
print("\nmodels ran cleanly; rows scored:", len(res))
champ.to_csv("validated_champions.csv", index=False)
agg.to_csv("validated_metrics.csv", index=False)
